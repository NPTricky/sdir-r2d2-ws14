...
def PTPtoConfiguration(robot, target_cfg, motiontype):
    ...
    if motiontype == "L":
        trajectory = linear_trajectory_interpolation(robot, target_cfg, 0.5, 1.5)
        return trajectory 
...
def generate_trajectory(start_cfg, target_cfg, velocity_limit, acceleration_limit, times_acc, times_dec, times_end):
    ...
    time_steps_max = int(time_steps_end.max()) + 1
...
def linear_trajectory_interpolation(robot, target_cfg, velocity = 5.0, acceleration=3.0):
     # determine the start and the end position in space
    start_matrix = kin.forward(robot.GetDOFValues())
    ...
    interpolated_path = linear_trajectory_path_interpolation( robot, start_pose, target_pose, sample_rate)
    
    configurations = linear_trajectory_configurations_generation( robot, target_cfg, interpolated_path)
    
    return linear_trajectory_generation(robot, configurations, sample_rate)

def linear_trajectory_path_interpolation(robot, start_pose, target_pose, sample_rate, velocity = 5.0, acceleration=3.0):
    ...
    time_steps_max = np.int(time_end / sample_rate) + 1
    
    interpolated_path = np.empty([time_steps_max, len(start_pose)])
...
def linear_trajectory_configurations_generation(robot, target_cfg, interpolated_path):
    ...
    for pose in (interpolated_path[::-1])[1:-2]:
        ...
        if np.pi/2 < np.fabs( inverse_configuration - configurations[-1] ).sum():
            inverse_configuration = get_best_inverse_angles_solution( configurations[-1], inverse_configurations)
    ...
    configurations.append(robot.GetDOFValues())
    
    return configurations[::-1]
    
def linear_trajectory_generation(robot, configurations, sample_rate):
    # generation of the trajectory path
    velocity = robot.GetDOFVelocityLimits()
    acceleration = robot.GetDOFAccelerationLimits()

    last_cfg = configurations[0]
    last_velocity = np.zeros(len(velocity))
    last_times_acc = np.zeros(len(velocity))
    
    i = 0
    trajectory = np.ones([1,6]) * last_cfg
    for cfg in configurations:
        ...
        trajectory = trajectory_concatenate( robot, trajectory, current_trajectory, 1 < i , last_velocity, last_times_acc, velocity_limit, times_acc)

        last_velocity = velocity_limit
        last_times_acc = times_acc
        last_cfg = cfg
        i = i + 1        
    return trajectory
    
def trajectory_concatenate(robot, trajectory_0, trajectory_1, gind_over, velocity_0, times_acc_0, velocity_1, times_acc_1):
        
        if not gind_over:
            return np.concatenate((trajectory_0, trajectory_1[1:]))
            
        
        time_dec0_to_end0 = times_acc_0.max()
        time_end0_to_acc1 = times_acc_1.max()
        time_end =  time_dec0_to_end0 + time_end0_to_acc1
                
        velocity = np.minimum( np.fabs(velocity_1 - velocity_0), robot.GetDOFVelocityLimits())
        ...
        time_steps_max = int(time_end / _SAMPLE_RATE)

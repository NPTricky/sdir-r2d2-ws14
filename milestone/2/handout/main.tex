...
\subsection{linear interpolation in ptp}
\subsubsection{Step 1}
\begin{frame}
...
\subsubsection{Step 2}
\begin{frame}
calculation of the waypoints of each axes
...
\subsection{linear interpolation in cartesian space}
\begin{frame}
\begin{columns}
  \column[c]{.50\textwidth}
  \begin{equation*}
  x_A = \left( \begin{array}{c} P_{x,A}\\ P_{y,A}\\ P_{z,A}\\ \hline \alpha_{x,A}\\ \beta_{y,A}\\ \gamma_{z,A} \end{array}\right) = \left( \begin{array}{c} P_{A}\\\hline\Theta_{A} \end{array}\right)
  \end{equation*}
  \column[c]{.50\textwidth}
  \begin{equation*}
  x_B = \left( \begin{array}{c} P_{x,B}\\ P_{y,B}\\ P_{z,B}\\ \hline \alpha_{x,B}\\ \beta_{y,B}\\ \gamma_{z,B} \end{array}\right) = \left( \begin{array}{c} P_{B}\\\hline\Theta_{B} \end{array}\right)
  \end{equation*}
\end{columns}
\begin{align*}
P & \text{ is the position in of a point in the world coordinate system} \\
...
\subsubsection{Step 1}
\begin{frame}
calculation of the path length $\hat{x}_e$ and the angle difference $\hat{x}_{\theta}$
\begin{eqnarray*}
\hat{x}_e & = & \left|P_{B} - P_{A}\right|\\
          & = & \sqrt{(P_{x,B} - P_{x,A})^2 + (P_{y,B} - P_{y,A})^2 + (P_{z,B} - P_{z,A})^2 }\\
\\
\hat{x}_\theta & = & \left|\Theta_{B} - \Theta_{A}\right|\\
...
\subsubsection{Step 2}
\begin{frame}
calculation the duration of the motion for the path length $t_{e}$ and the angle difference $t_{\theta}$
\begin{eqnarray*}
t_{e} = \frac{\hat{x}_{e}}{\hat{v}_{e}}+\frac{\hat{v}_{e}}{\hat{a}_{e}} \\
\\
t_{\theta} = \frac{\hat{x}_{\theta}}{\hat{v}_{\theta}}+\frac{\hat{v}_{\theta}}{\hat{a}_{\theta}}
\end{eqnarray*}
the speed and the acceleration is set by the operator
\end{frame}
...
\begin{eqnarray*}
t_{max} = max\left(t_{e}, t_{\theta}\right)\\
\end{eqnarray*}
\begin{itemize}
\item $t_{\theta} \le t_{e}$\\
\begin{eqnarray*}
v & = & \frac{t_{max} \cdot \hat{a}_{e}}{2}-\sqrt{\frac{t_{max}^2 \cdot \hat{a}_{e}^2}{4}-\hat{a}_{e}\cdot \hat{x}_{e}} \\
t_{a} & = & \frac{v}{\hat{a}_{e}}
\end{eqnarray*}
\item $t_{e} \leq t_{\theta}$\\
\begin{eqnarray*}
v & = & \frac{t_{max} \cdot \hat{a}_{\theta}}{2}-\sqrt{\frac{t_{max}^2 \cdot \hat{a}_{\theta}^2}{4}-\hat{a}_{\theta}\cdot \hat{x}_{\theta}} \\
...
\begin{frame}
the position $x(t)$ to time $t$
\begin{eqnarray*}
x(t) & = & x_{a} + \frac{x_{e}(t)}{\hat{x}_{e}} \cdot \left(x_{b} - x_{a} \right)\\
x_{e}(t) & = & 
\begin{dcases}
\frac{1}{2} \hat{a}_{e} \cdot t & t  \leq t_{a} \\
v \cdot t - \frac{1}{2} \cdot \frac{v^2}{\hat{a}_{e}} & t_{a} \le t \leq t_{max} - t_{a} \\
v \cdot (t_{max} - t_{a}) - \frac{\hat{a}_{e}}{2} \cdot (t_{max} - t)^2 & t_{max} - t_{a} \le t \leq t_{max}\\
\end{dcases}
\end{eqnarray*}
\end{frame}
...
\begin{eqnarray*}
\Theta(t) & = & \Theta_{a} + \frac{x_{\Theta}(t)}{\hat{x}_{\Theta}} \cdot \left(\Theta_{b} - \Theta_{a} \right)\\
x_{\Theta}(t) & = & 
\begin{dcases}
\frac{1}{2} \hat{a}_{\Theta} \cdot t & t  \leq t_{a} \\
v \cdot t - \frac{1}{2} \cdot \frac{v^2}{\hat{a}_{\Theta}} & t_{a} \le t \leq t_{max} - t_{a} \\
v \cdot (t_{max} - t_{a}) - \frac{\hat{a}_{\Theta}}{2} \cdot (t_{max} - t)^2 & t_{max} - t_{a} \le t \leq t_{max}
\end{dcases}
\end{eqnarray*}
\end{frame}
